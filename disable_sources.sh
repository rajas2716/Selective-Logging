...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            echo "${SOURCE_NAME} is already disabled"
        else
            echo "Disabling ${SOURCE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsSourceConfigurations/${DEFAULT_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${SOURCE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${SOURCE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsSourceConfigurations/${SPARK_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${SOURCE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${SOURCE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsSourceConfigurations/${INTERACTIVE_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${SOURCE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${SOURCE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsSourceConfigurations/${HBASE_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${SOURCE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${SOURCE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsSourceConfigurations/${HADOOP_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${SOURCE_NAME}"   
        fi
        fi
    done
