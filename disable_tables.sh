...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            echo "${TABLE_NAME} is already disabled"
        else
            echo "Disabling ${TABLE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsOutputConfigurations/${DEFAULT_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${TABLE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${TABLE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsOutputConfigurations/${SPARK_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${TABLE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${TABLE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsOutputConfigurations/${INTERACTIVE_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${TABLE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${TABLE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsOutputConfigurations/${HBASE_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${TABLE_NAME}"   
        fi
        fi
    done
...
then
    ...
    then
        ...
        if test -f "$FILE"; then
            ...
            echo "Disabling ${TABLE_NAME}"
            sudo mv "/etc/td-agent/CurrentLogAnalyticsOutputConfigurations/${HADOOP_FILE_NAMES[$i]}" $FILE
            echo "Successfully Disabled ${TABLE_NAME}"   
        fi
        fi
    done
